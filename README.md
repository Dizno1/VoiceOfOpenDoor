...
- **What's built:** `Tools/Audio Processing/analyze.py` (objective audio report); `Tools/Dataset Utilities/build_manifest.py` (structured manifest); `App/` — a full 8-tool nav skeleton (Home, Recordings, Analyze, Transcripts, Segments, Train, Generate Speech, Settings), with Home, Recordings, Analyze, and Settings fully working. As of Aug 5, 2026, all working data (recordings, manifest) lives outside the repository in a user-configured folder, not committed to GitHub - see `App/README.md`'s External Data Architecture section and `App/backend/local_data.py`.
