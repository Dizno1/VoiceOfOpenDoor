...
## Priority-Order Fix and Centralized Focus Management (Aug 5, 2026, same day)

**The bug, reproduced exactly before fixing it:** Dean reported that after launch, the console correctly said "Scan complete: 20 recordings already registered, nothing new found," but Home still said "No recordings currently require attention" - false, since all 20 were unanalyzed. Root cause: `compute_workflow_status()` never checked staleness at all (a stale analysis's old flags were trusted as still "clear"), and Home's recommendation logic only considered Needs Review and Analysis Failed, never Not Analyzed. Reproduced the exact scenario in a disposable folder (pre-registered recordings with `analysis: None`, matching what an older corpus looks like from before analysis-on-discovery existed) and confirmed the same false "nothing needs attention" message before fixing anything.

**Fix:** `compute_workflow_status()` now checks staleness first - a stale analysis is treated as Not Analyzed, not trusted. Home's recommendation is a real priority chain, not just "issues vs. nothing": Not Analyzed/stale first ("N recordings are waiting for analysis" / "Analyze N recordings," linking straight to Analyze with a focus hint on the right section), then Analysis Failed ("Review analysis failures"), then Needs Review ("Review recordings with possible issues"), and only "No recordings currently require attention" when none of those are true. Verified the entire chain for real: unanalyzed → ran the recommended batch analysis → correctly fell through to the next tier (one recording flagged) → would fall through again to "nothing needs attention" once that's resolved too, same mechanism already proven earlier this session.

**Focus management, rebuilt as one system, not patched:** every template now follows exactly two rules - the H1 always carries `id="page-heading" tabindex="-1"` (the correct target for ordinary navigation), and when a page needs to focus something else because the H1 alone doesn't convey what just happened (a save confirmation, a delete result, a batch-analysis outcome), that element carries `id="page-focus-target" tabindex="-1"` instead. `app.js` is now the single place this is decided - one `load` listener, one delay, one fallback rule (`page-focus-target` if present, else `page-heading`, never `main`/`body`/nav/skip-link/Home button) - replacing the old `#status-message`/`#confirm-heading` pattern that had grown inconsistently across templates over the course of this project. Applied identically to all 11 templates, not selectively.

One specific case built to match the spec directly: Home's "Analyze N recordings" link passes `?focus=needing` through the URL (server-rendered query state, not a client-side guess), and the Analyze route uses it to put `page-focus-target` on the "Recordings Needing Analysis" heading instead of the generic H1 - verified this renders correctly rather than just asserting it should.

**Tested for real, not just by construction:** the exact reported scenario end to end (console message, Home's corrected wording, the working "Analyze N recordings" link, the focus hint landing on the right heading, the priority chain correctly advancing after the batch completes), and classification-save focus targeting on the recording detail page (confirmed the status message becomes the target, not the H1, and both elements are correctly present and distinguished). Not individually clicked through: every named permutation in Dean's full test list - NVDA specifics aren't testable in this sandbox at all, and several navigation-focus/back-button cases are covered by the uniform system's construction rather than individually exercised. The system is applied consistently everywhere the same way it was verified in the spots that were checked, but that's a claim about consistency, not a substitute for Dean's own JAWS pass across the full list.
